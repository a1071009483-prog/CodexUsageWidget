...
| 7.4 | Manual install/first-run/startup/pause/upgrade/rollback/uninstall | 🖥️ Manual | ⏳ | See steps below |
| 7.5 | Authenticated read-only smoke test | `ReadOnlyAuthenticatedSmokeTest.cs` | ⏳ | See steps below |
| 7.6 | Real activation acceptance test | `RealActivationAcceptanceTest.cs` | ⏳ | See steps below |
| 7.7 | Full automated suite and final acceptance matrix | 🖥️ Manual + automated | ⏳ | See steps below |
...
- [ ] The floating widget appears and is always on top of ordinary windows.
- [ ] The tray icon appears and the menu contains Show/Hide, Refresh Now,
      Pause/Resume Automatic Triggering, Start with Windows, Audit, Reconnect,
      and Exit. No force-consume command is present.
- [ ] Hide the widget; the tray icon and process remain.
- [ ] Pause automatic triggering; monitoring continues but automatic activation
      is disabled.
- [ ] Resume automatic triggering.
- [ ] Disable **Start with Windows** from the tray; verify the registry entry is
      removed under `HKCU:\Software\Microsoft\Windows\CurrentVersion\Run`.
- [ ] Re-enable **Start with Windows**.
- [ ] Upgrade:
  ```powershell
  .\scripts\upgrade.ps1
  ```
...
- [ ] Uninstall keeping local data:
  ```powershell
  .\scripts\uninstall.ps1
  ```
- [ ] Verify `%LOCALAPPDATA%\CodexUsageWidget\state.db` still exists.
- [ ] Uninstall removing local data:
  ```powershell
  .\scripts\uninstall.ps1 -RemoveLocalData
  ```
- [ ] Verify `%LOCALAPPDATA%\CodexUsageWidget\` is removed or contains only
...
- [ ] Test discovers the five-hour bucket (`windowDurationMins = 300`).
- [ ] Test discovers the weekly bucket (duration close to 10080 minutes).
- [ ] First snapshot is fresh within 60 seconds.
- [ ] Countdown decreases between snapshots.
- [ ] Reconciliation refreshes `SyncedAt` within 75 seconds.
- [ ] No `turn/start` call is issued.
...
- [ ] Real-time values update within one second of a notification.
- [ ] Countdown text updates every second.
- [ ] After 120 seconds without a successful read, the widget shows stale data.
- [ ] After reconnect, a fresh read clears the stale state.
- [ ] Crash recovery: with a live suppression lock, restart the widget and confirm
      no new `turn/start` is issued for the same window.
- [ ] Model fallback: if a lightweight model is unavailable and the default model
      is selected, the audit row records the fallback model ID.
- [ ] After successful activation, no further generation occurs until the current

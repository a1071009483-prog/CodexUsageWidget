...
- [x] 7.5 Implement a read-only authenticated smoke test harness that confirms real five-hour/weekly mapping, startup freshness, ~30-second reconciliation (kept under 60 seconds to satisfy the App Server idle timeout), countdown behavior, and zero model turns.
  Evidence: passed on Windows on 2026-08-27 against authenticated `codex-cli 0.148.0-alpha.9`; the real response exposed the Codex five-hour primary and weekly secondary windows alongside a separate `base_model_inference` weekly limit. The widget maps the paired Codex windows, reconciles in 34.46 seconds, and issues zero model turns.
- [ ] 7.6 With explicit user approval and a fully unused five-hour window, run exactly one real activation test and verify a future `resetsAt`, temporary-task cleanup, redacted audit data, and zero later `turn/start` calls in that guarded period.  
  See `tests/CodexUsageWidget.AcceptanceTests/Activation/RealActivationAcceptanceTest.cs` and `docs/acceptance-matrix.md`. **Pending a fully unused five-hour window. The connected account now exposes both target buckets, but the live five-hour window is already active and therefore correctly fails the exact-zero/unused eligibility gate. No turn was consumed by this verification.**
- [x] 7.7 Execute the full automated suite and manually check the final acceptance matrix: real-time values, one-second notification display, 60-second maximum reconciliation, 120-second stale state, at-most-once activation, crash recovery, model fallback, and no post-success generation.  
  Evidence: `dotnet test CodexUsageWidget.sln -c Release` passes with Core 62/62, App 36/36, Infrastructure 163/163 and no test excluded; the two explicitly gated acceptance tests skip when their environment variables are absent. See `docs/acceptance-matrix.md`.

...
- [ ] 7.6 With explicit user approval and a fully unused five-hour window, run exactly one real activation test and verify a future `resetsAt`, temporary-task cleanup, redacted audit data, and zero later `turn/start` calls in that guarded period.  
  See `tests/CodexUsageWidget.AcceptanceTests/Activation/RealActivationAcceptanceTest.cs` and `docs/acceptance-matrix.md`. **Pending ChatGPT authentication (`codex login`) — the test failed fast at account identity resolution and did not consume a turn.**

...
- [ ] 7.5 Implement a read-only authenticated smoke test harness that confirms real five-hour/weekly mapping, startup freshness, ~30-second reconciliation (kept under 60 seconds to satisfy the App Server idle timeout), countdown behavior, and zero model turns. The harness is implemented and previously passed against a real Codex CLI that exposed a five-hour bucket; the connected ChatGPT-backed account currently returns only a weekly (`windowDurationMins = 10080`) bucket, so the test fails at the five-hour availability assertion. (Execution requires Windows + authenticated Codex CLI that exposes a five-hour bucket.)
- [ ] 7.6 With explicit user approval and a fully unused five-hour window, run exactly one real activation test and verify a future `resetsAt`, temporary-task cleanup, redacted audit data, and zero later `turn/start` calls in that guarded period.  
  See `tests/CodexUsageWidget.AcceptanceTests/Activation/RealActivationAcceptanceTest.cs` and `docs/acceptance-matrix.md`. **Pending an account/plan that exposes a five-hour Codex bucket — the connected ChatGPT-backed account currently returns only a weekly (`windowDurationMins = 10080`) bucket, so the test cannot reach the activation gate. The authentication-state misclassification that previously blocked identity resolution was fixed in commit `06e5e60`. No turn was consumed.**
- [x] 7.7 Execute the full automated suite and manually check the final acceptance matrix: real-time values, one-second notification display, 60-second maximum reconciliation, 120-second stale state, at-most-once activation, crash recovery, model fallback, and no post-success generation.  
  Evidence: `dotnet test CodexUsageWidget.sln -c Release` passes with Core 61/61, App 34/34, Infrastructure 161/161 and no test excluded; the previously flaky supervisor test was stabilized in commit `b0a0b31`. See `docs/acceptance-matrix.md`.

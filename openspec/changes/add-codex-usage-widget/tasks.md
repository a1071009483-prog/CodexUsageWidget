...
- [x] 7.5 Implement a read-only authenticated smoke test harness that confirms real five-hour/weekly mapping, startup freshness, ~30-second reconciliation (kept under 60 seconds to satisfy the App Server idle timeout), countdown behavior, and zero model turns. Previously executed and passed against a real Codex CLI that exposed a five-hour bucket; the connected ChatGPT-backed account currently returns only a weekly (`windowDurationMins = 10080`) bucket, so the test fails at the five-hour availability assertion. (Execution requires Windows + authenticated Codex CLI that exposes a five-hour bucket.)
